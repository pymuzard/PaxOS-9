...
void applications::launcher::draw()
{
    ...
    // TEST PYM
    testSlider = new Slider(55, 110, 210, 30, 0, 100, 50);
    testSlider->displayValue(true);
    testSlider->setFormatPercentage(true);
    launcherWindow->addChild(testSlider);
    ...
    brightnessSliderBox->setBackgroundColor(COLOR_BLUE);
    ...
    for (const auto &app : AppManager::appList)
    {
        ...
        applicationsIconsMap.insert({box,
                                     app});
