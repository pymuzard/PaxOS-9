...
std::string dump(const sol::object &obj)
{ // NOLINT(*-no-recursion)
    if (obj.is<sol::table>())
    {
...
namespace paxolua::lib
{
    ...
    {
        libsystem::log("Loading CoreLibrary");
        ...
        lua["print"] = [&](const sol::object &obj)
        {
            ...
            {
                print(std::to_string(obj.as<int>()));
            }
            else if (obj.is<bool>())
            {
                print(std::to_string(obj.as<bool>()));
            }
            else if (obj.as<sol::table>())
            {
                print(dump(obj));
            }
        };
