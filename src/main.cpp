...
void setup()
{
    ...
    // read saved Config and apply it
    if (libsystem::paxoConfig::getBrightness() == 0)
    {
        libsystem::log("no brightness setting saved - set defaut");
        libsystem::paxoConfig::setBrightness(69, true);
    }
    libsystem::paxoConfig::setBrightness(libsystem::paxoConfig::getBrightness(), false);

    if (libsystem::paxoConfig::getStandBySleepTime() == 0)
    {
        libsystem::log("no SleepTime setting saved - set defaut");
        libsystem::paxoConfig::setStandBySleepTime(10000, true);
    }
    libsystem::paxoConfig::setStandBySleepTime(libsystem::paxoConfig::getStandBySleepTime(), false);
    ...
    GSM::ExternalEvents::onIncommingCall = []()
    {
        eventHandlerApp.setTimeout(new Callback<>([]()
                                                  { AppManager::get(".receivecall")->run(false); }),
                                   0);
    };
